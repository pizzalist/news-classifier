      ...
      <div class="modalPage">
        ...
        <div class="completeBtn">
          <div class="ButtonArea">
            <BlueButton ButtonText="완료" @click="showCategorizedItems()" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async showCategorizedItems() {
      ...
      // Iterate through the newsItems and extract categoryName and title
      this.newsItems.forEach((item) => {
        const categoryName = this.getCategoryName(item.category_id);

        // Push categoryName and title to the cliped_news array
        cliped_news.push({
          categoryName,
          title: item.title,
        });
